...
#################################  HELPER FUNCTIONS  #################################
	...
	def load(self):
		self.background = pygame.image.load('pacman_maze.png')
		...
		self.max_score = len(self.coins)


	def make_enemies(self):
		for i, pos in enumerate(self.e_pos):
			self.enemies.append(Enemy(self,vec(pos),i))
...
#################################  INTRO  FUNCTIONS  #################################
	...
	def start_draw(self):
		...
		self.draw_text('HIGH SCORE : {}'.format(self.best_score),self.screen, [4,0], START_TEXT_SIZE, WHITE, START_FONT)
...
#################################  PLAYING  FUNCTIONS  #################################
	...
	def playing_update(self):
		...
		if self.player.current_score == self.max_score:
			with open('score.txt', 'w') as file:
				if(self.player.current_score > self.best_score):
					file.write("BEST_SCORE: {}".format(self.player.current_score))
					self.best_score = self.player.current_score
			self.state = 'victory'
	...
	def playing_draw(self):
		...
		self.draw_text('HIGH SCORE : {}'.format(self.best_score), self.screen, [WIDTH//2+60,0], 18, WHITE, START_FONT)
		self.player.draw()
		for enemy in self.enemies:
			enemy.draw()
		pygame.display. update()
	...
	def draw_coins(self):
		for coin in self.coins:
			self.screen.blit(COIN_IMG, (
								int(coin.x * self.cell_width) + self.cell_width//2 + TOP_BOTTOM_BUFFER//2 - 5, 
...
#############################  GAME OVER FUNCTIONS  #############################
	...
	def game_over_draw(self):
		...
		self.draw_text("Press the escape button to QUIT", self.screen, [WIDTH//2, HEIGHT//2], 36, (190,190,190), "arial", centered=True)
		self.draw_text("Press SPACE BAR to PLAY AGAIN", self.screen, [WIDTH//2, HEIGHT//1.5], 36, (190,190,190), "arial", centered=True)
		pygame.display.update()
...
##############################  VICTORY FUNCTIONS  ##############################
	...
	def victory_draw(self):
		self.screen.fill(BLACK)
		self.draw_text("VICTORY !!!!", self.screen, [WIDTH//2, 100], 36, GOLD, "arial", centered=True)
		self.draw_text("Press the escape button to QUIT", self.screen, [WIDTH//2, HEIGHT//2], 36, (190,190,190), "arial", centered=True)
		self.draw_text("Press SPACE BAR to PLAY AGAIN", self.screen, [WIDTH//2, HEIGHT//1.5], 36, (190,190,190), "arial", centered=True)
		pygame.display.update()